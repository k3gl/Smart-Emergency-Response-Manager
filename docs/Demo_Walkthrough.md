# PRVIA — Full Demo Walkthrough (step by step)
...
| Admin (seeded) | admin@prvia.com | (your admin pass) |
| Unit (admin provisions it live, or pre-made) | unit.demo@prvia.com | 123456 |
...
6. Tap **Create Unit**. In the dialog, type:
   ...
   - **Unit code:** `POL-009`
   - **Login email (unique):** `unit.demo@prvia.com`
...
1. **Switch to the phone** (Unit Dashboard, logged in as the unit).
   - If not logged in yet, log in now with `unit.demo@prvia.com` / `123456`.
